		...
		<%} else {
			try {
				...
				if (result.next()) {
			    	...
			    	<table>
			    	<tr>    
						<td>Auction Id</td> <td><%= result.getString("s.auction_id") %></td>
					</tr>
					<tr>
						<td>Product Id</td> <td><%= result.getString("p.product_id") %></td>
					</tr>
					<tr>
						<td>Brand</td> <td><%= result.getString("p.brand") %></td>
					</tr>
					<tr>
						<td>Model</td> <td><%= result.getString("p.model") %></td>
					</tr>
					<tr>
						<td>Initial Price</td> <td><%= result.getString("s.initial_price") %></td>
					</tr>
					<tr>
						<td>OS</td> <td><%= result.getString("p.operating_system") %></td>
					</tr>
					<tr>
						<td>Screen Size</td> <td><%= result.getString("p.screen_size") %></td>
					</tr>
					<tr>
						<td>Condition</td> <td><%= result.getString("p.cond") %></td>
					</tr>
					<tr>
						<td>Category</td> <td><%= result.getString("p.category") %></td>
					</tr>
					<tr>
						<td>Start Date & Time</td> <td><%= result.getString("s.start_date") %> @ <%= result.getString("s.start_time") %></td>
					</tr>
					<tr>
						<td>End Date & Time</td> <td><%= result.getString("s.closing_date") %> @ <%= result.getString("s.closing_time") %></td>
					</tr>
					<tr>
						<td>Status</td> 
				...
				%>
				<br>
				<form method='post' action="auction-details.jsp"> <!-- Can send the auction_id somewhere -->
				...
				String history_query = "SELECT * FROM Buys b JOIN Sells s USING (auction_id) WHERE auction_id = " + auction_id + " ORDER BY b.current_bid DESC";
				...
				String template_query = "SELECT * FROM Sells s JOIN Product p USING (product_id) WHERE s.start_date < '"+ last_month + "' AND s.closing_date > '"+ this_month + "'";
